    ...
    SurfaceInfo& info)
{
    ...
    const float3 v0 =
        transform_position(object_to_world, params.vertices[idx.x]);
    const float3 v1 =
        transform_position(object_to_world, params.vertices[idx.y]);
    const float3 v2 =
        transform_position(object_to_world, params.vertices[idx.z]);
    ...
    const float u, const float2& v, float3& le, float3& n, float& pdf)
{
    ...
    const Matrix3x4& object_to_world =
        params.object_to_world[light.instance_idx];
    const Matrix3x4& world_to_object =
        params.world_to_object[light.instance_idx];
...
sample_position_on_directional_light(const float2& u)
{
    ...
    const float3 p =
        DIRECTIONAL_LIGHT_DISTANCE * params.directional_light->dir +
...
extern "C" __global__ void __miss__light()
{
    ...
    if (params.ibl) { payload->le = fetch_ibl(payload->direction); }
    else if (params.arhosek)
    {
        payload->le = evaluate_arhosek_sky(payload->direction);
    }
    else { payload->le = params.bg_color; }
}
    ...
    if (payload->firsthit)
    {
        ...
        if (has_emission(material))
        {
            payload->radiance += payload->throughput *
                                 get_emission(material, surf_info.texcoord);
            payload->done = true;
            return;
        }
    }
    ...
    // light sampling
    {
        ...
        if (params.directional_light)
        {
            const float3 shadow_ray_direction =
                normalize(sample_position_on_directional_light(
                              sample_2d(payload->sampler)) -
                          shadow_ray_origin);

            ShadowPayload shadow_payload;
            trace_shadow(params.ias_handle, shadow_ray_origin,
                         shadow_ray_direction, 0.0f, 1e9f, &shadow_payload);

            if (shadow_payload.visible)
            {
                const float3 wi = world_to_local(shadow_ray_direction, tangent,
                                                 normal, bitangent);
        ...
        if (params.n_lights > 0)
        {
            ...
            const float3 p = sample_position_on_light(
                sample_1d(payload->sampler), sample_2d(payload->sampler), le, n,
                pdf_area);
            // const float3 p = sample_position_on_light(
            //     sample_1d(payload->sampler),
            //     blue_noise_2d(payload->sampler.blue_noise_state), le, n,
            //     pdf_area);
    ...
    // BSDF sampling
    {
        ...
        if (light_payload.hit)
        {
            ...
            const float pdf_area =
                1.0f / (params.n_lights * light_payload.area);
